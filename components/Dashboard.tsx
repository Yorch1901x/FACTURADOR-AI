  ...
  const StatCard = ({ title, value, icon: Icon, colorClass, subtext }: any) => (
    <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 hover:shadow-md transition-shadow duration-200">
      <div className="flex justify-between items-start mb-4">
        <div className={`p-3 rounded-xl bg-gray-50 border border-gray-100 text-black`}>
          <Icon size={24} />
        </div>
        <span className="flex items-center text-xs font-bold text-black bg-gray-100 px-2 py-1 rounded-full border border-gray-200">
    ...
    <div className="space-y-8 animate-fade-in pb-10">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <button
          ...
          disabled={loadingAi}
          className="w-full md:w-auto flex items-center justify-center gap-2 bg-black text-white px-5 py-2.5 rounded-xl hover:bg-gray-800 transition-all disabled:opacity-70 shadow-lg shadow-gray-200 active:scale-95 border border-black"
        >
      ...
      {aiAnalysis && (
        <div className="bg-gray-50 p-6 rounded-2xl border border-gray-200 shadow-sm relative overflow-hidden">
          <div className="absolute top-0 right-0 -mt-4 -mr-4 w-24 h-24 bg-gray-200 rounded-full blur-3xl opacity-50"></div>
          <h3 className="text-lg font-bold text-gray-900 flex items-center gap-2 mb-4 relative z-10">
            <Sparkles size={20} className="text-black" /> Análisis Inteligente
          </h3>
          <div className="prose prose-sm prose-gray max-w-none text-gray-700 relative z-10 bg-white p-4 rounded-xl border border-gray-100">
            {aiAnalysis.split('\n').map((line, i) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 flex flex-col">
          <div className="flex justify-between items-center mb-6">
             ...
             <span className="text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded-md font-medium">Últimos 7 movimientos</span>
          </div>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData}>
                  ...
                  <Tooltip 
                    ...
                    itemStyle={{color: '#fff'}}
                  />
                  <Bar dataKey="amount" fill="#111827" radius={[4, 4, 0, 0]} barSize={40} />
                </BarChart>
              </ResponsiveContainer>
            ) : (
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 flex flex-col">
           <div className="flex justify-between items-center mb-6">
             ...
             <span className="text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded-md font-medium">Tiempo Real</span>
           </div>
           <div className="h-72 w-full">
             {chartData.length > 0 ? (
               <ResponsiveContainer width="100%" height="100%">
                <LineChart data={chartData}>
                  ...
                  <Line 
                    ...
                    activeDot={{r: 6, stroke: '#000000', strokeWidth: 0, fill: '#000'}} 
                  />
                </LineChart>
               </ResponsiveContainer>
             ) : (
